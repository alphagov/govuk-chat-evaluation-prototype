...
def run_deepeval_evaluation(cases: list[EvaluationTestCase], metrics: list[BaseMetric], n_runs: int = 1, **kwargs) -> list[TestResult]:
    """"
    Run the Deepval evaluation on the given models and metrics
    ...
    Returns:
        list[dict]: List of evaluation results for each model

    """

    llm_test_cases = [case.to_llm_test_case() for case in cases]
    # evaluation_dataset = EvaluationDataset(test_cases=llm_test_cases) # type: ignore
    ...
    with Timer("Running Deepval Evaluation"):
        ...
        for i in range(n_runs):
            ...
            evaluation_run = deepeval_evaluate(
                test_cases=cast(list[LLMTestCase | MLLMTestCase], llm_test_cases), # hint to type checker: this is a List[LLMTestCase | MLLMTestCase], even though it's just List[LLMTestCase]
                metrics=metrics,
                **kwargs,  # pass additional arguments dynamically 
            )
    ...
    flat_evaluation_runs = [results for run in all_evaluation_runs for results in run]

    return flat_evaluation_runs
