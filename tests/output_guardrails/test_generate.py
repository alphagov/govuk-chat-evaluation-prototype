...
def test_generate_models_to_evaluation_models_runs_expected_rake_task(
    ...
    generate_inputs_to_evaluation_results("openai", "answer", generate_inputs)

    run_rake_task_mock.assert_called_with(
        "evaluation:generate_output_guardrail_response[openai,answer]",
        {"INPUT": "Question 1"},
    )
...
def test_generate_and_write_dataset(mock_input_data, mock_project_root):
    path = generate_and_write_dataset(
        mock_input_data, "openai", "answer", mock_project_root
    )
    assert path.exists()
