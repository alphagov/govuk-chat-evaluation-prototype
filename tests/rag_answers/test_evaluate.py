...
from govuk_chat_evaluation.rag_answers.data_models import EvaluationResult, RunMetricOutput, EvaluationTestCase, StructuredContext
from govuk_chat_evaluation.rag_answers.evaluate import (AggregatedResults, evaluate_and_output_results, DEEPEVAL_EVAL_PARAMETERS)


def test_aggregated_results_stats():
    eval_results = [
        EvaluationResult(name="Test1",
            input="Is Vat a tax?",
            actual_output="Yes",
    ...
    assert set(df_avg.columns.get_level_values(0)) == {"input", "name", "mean", "mean", "std", "std"}
    assert set(df_avg.columns.get_level_values(1)) == {"", "bias", "faithfulness", "bias", "faithfulness"}
...
def test_evaluate_and_output_results_function_calls(tmpdir, mock_evaluation_config):
    ...
    # set up all mocks
    with patch('govuk_chat_evaluation.rag_answers.evaluate.jsonl_to_models', return_value=mock_test_cases) as mock_jsonl_to_models:
        with patch('govuk_chat_evaluation.rag_answers.evaluate.run_deepeval_evaluation', return_value=mock_deepeval_output) as mock_run_eval:
            with patch('govuk_chat_evaluation.rag_answers.evaluate.convert_deepeval_output_to_evaluation_results', return_value=mock_results) as mock_convert:
                with patch('govuk_chat_evaluation.rag_answers.evaluate.AggregatedResults') as MockAggregatedResults:
                    ...
                    mock_jsonl_to_models.assert_called_once_with(eval_data_path, EvaluationTestCase)
                    ...
                    assert call_args["metrics"] == mock_evaluation_config.metric_instances()
